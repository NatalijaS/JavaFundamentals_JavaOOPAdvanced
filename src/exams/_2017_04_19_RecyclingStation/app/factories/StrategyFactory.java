...
public class StrategyFactory<T> implements Factory<GarbageDisposalStrategy> {
    ...
    @Override
    public GarbageDisposalStrategy create(String... args) throws ClassNotFoundException,
        ...
        for (int i = 0; i < wasteParameterTypes.length; i++) {
            Class<?> parameterType = wasteParameterTypes[i].isPrimitive() ?
                    getWrapper(wasteParameterTypes[i]) : wasteParameterTypes[i];
